...
import { 
  ...
  FileDown, CheckCircle, ShieldCheck, Users, UserPlus, UserMinus, Settings
} from 'lucide-react';

const ACTIVE_CLINIC_ID = 'local-demo-clinic';
const DEFAULT_SPECIALTIES = ["Internal Medicine", "Surgery", "Oncology", "Neurology", "ER & Critical Care", "Cardiology", "Dermatology"];
...
export const StaffDashboard: React.FC<StaffDashboardProps> = ({ onLogout, doctor }) => {
  ...
  const [newStaff, setNewStaff] = useState({ name: '', specialty: DEFAULT_SPECIALTIES[0], pin: '' });
  ...
  const handleAddSpecialty = (e: React.FormEvent) => {
    ...
    setNewStaff({ ...newStaff, specialty: customSpecialty });
    setCustomSpecialty('');
    ...
    <div className="max-w-7xl mx-auto pb-20 p-4 relative">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-sm w-full overflow-hidden animate-in zoom-in-95">
            <div className="p-8 text-center">
              ...
              <p className="text-sm text-slate-500">Archive <span className="font-bold text-slate-900">{dischargeTarget.name}</span>'s active record?</p>
            </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{isAdminPortal ? 'Clinic Admin' : doctor.name}</h1>
            <p className="text-indigo-600 font-medium text-sm">{isAdminPortal ? 'Caseload & Staff Control' : doctor.specialty}</p>
          </div>
        </div>
      ...
      {/* 2. ADMIN TOOLS - Specialty Logic Included */}
      {isAdminPortal && (
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
            ...
            <form onSubmit={async (e) => {
              ...
              <div className="flex gap-2">
                <select value={newStaff.specialty} onChange={(e) => setNewStaff({...newStaff, specialty: e.target.value})} className="flex-1 px-5 py-3 bg-slate-50 border-none rounded-2xl text-sm outline-none focus:ring-2 focus:ring-amber-500 font-bold text-slate-700">
              ...
              <button type="submit" className="w-full py-4 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-2xl shadow-lg shadow-amber-100 transition-all">Onboard Provider</button>
            </form>
            
            <div className="mt-8 pt-6 border-t border-slate-50">
               <label className="text-[10px] font-black uppercase text-slate-400 mb-3 block">Manage Specialties</label>
               <form onSubmit={handleAddSpecialty} className="flex gap-2">
                  <input type="text" value={customSpecialty} onChange={(e) => setCustomSpecialty(e.target.value)} placeholder="New Specialty Category..." className="flex-1 px-4 py-2 bg-slate-50 rounded-xl text-xs outline-none border border-slate-100" />
                  <button type="submit" className="px-4 py-2 bg-slate-100 text-slate-600 rounded-xl text-xs font-bold hover:bg-indigo-50 hover:text-indigo-600 transition-all"><Plus size={14}/></button>
               </form>
            </div>
          </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-8 mb-12">
          ...
          <form onSubmit={async (e) => {
            ...
            <div className="md:col-span-3"><button type="submit" className="w-full bg-indigo-600 text-white font-bold py-3 rounded-2xl transition-all shadow-lg shadow-indigo-100">Check In</button></div>
          </form>
      </div>

      {/* 4. BALANCED NAVIGATION BAR */}
      <div className="mb-8 px-2 flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="flex gap-4 bg-white/60 p-1.5 rounded-[1.5rem] border border-slate-100 shadow-sm">
          <button onClick={() => setViewMode('active')} className={`px-10 py-2.5 rounded-2xl text-sm font-bold transition-all ${viewMode === 'active' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-slate-400 hover:text-slate-600'}`}>Active Patients</button>
          <button onClick={() => setViewMode('discharged')} className={`px-10 py-2.5 rounded-2xl text-sm font-bold transition-all ${viewMode === 'discharged' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-slate-400 hover:text-slate-600'}`}>Discharged</button>
        </div>
        
        {isAdminPortal && (
          <div className="flex items-center gap-4 bg-white px-5 py-2.5 rounded-2xl border border-slate-100 shadow-sm">
            <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Filter by Doctor:</span>
            <select value={adminDoctorFilter} onChange={(e) => setAdminDoctorFilter(e.target.value)} className="bg-transparent text-sm font-bold text-indigo-600 outline-none cursor-pointer min-w-[150px]">
              ...
              <div className="p-8 lg:p-10">
                <div className="flex flex-col lg:flex-row justify-between items-start gap-8 mb-10">
                  ...
                  <div className="flex flex-wrap gap-2 w-full lg:w-auto">
                    <a href={clientLink} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-5 py-3 bg-slate-50 hover:bg-slate-100 text-slate-700 rounded-xl text-sm font-bold border border-slate-100 transition-all"><Eye size={18}/> Preview</a>
                    <button onClick={() => handleSendSMS(patient)} disabled={sendingSms[patient.id]} className="flex items-center gap-2 px-5 py-3 bg-slate-50 hover:bg-slate-100 text-slate-700 rounded-xl text-sm font-bold border border-slate-100 transition-all">{sendingSms[patient.id] ? <Loader2 className="animate-spin" size={18}/> : <Send size={18}/>} SMS Update</button>
                    <button onClick={() => setAdvancedOpen(prev => ({ ...prev, [patient.id]: !prev[patient.id] }))} className="flex items-center gap-2 px-5 py-3 bg-slate-50 hover:bg-slate-100 text-slate-700 rounded-xl text-sm font-bold border border-slate-100 transition-all">Advanced {advancedOpen[patient.id] ? <ChevronUp size={18}/> : <ChevronDown size={18}/>}</button>
                  </div>
                </div>
                  ...
                  <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200 mb-10 animate-in slide-in-from-top-4">
                    ...
                    <div className="flex flex-wrap justify-between items-center pt-8 border-t border-slate-200 gap-y-8">
                      <div className="flex flex-wrap items-center gap-x-12 gap-y-8">
                        ...
                        <button onClick={() => handleCopy(clientLink, `${patient.id}-link`)} className="flex flex-col text-left group">
                           <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter mb-0.5">Portal Access</span>
                           <div className="flex items-center gap-2 text-sm font-bold text-slate-700 group-hover:text-indigo-600 transition-all">
                              Copy Direct Link {copiedField === `${patient.id}-link` ? <Check size={14} className="text-emerald-500" /> : <Copy size={14} className="text-slate-300" />}
                           </div>
                        </button>
                        <div className="flex flex-col"><span className="text-[10px] font-bold text-slate-400 uppercase mb-0.5">Patient ID</span><span className="font-mono text-sm font-bold text-slate-700">{patient.id}</span></div>
                        <div className="flex flex-col"><span className="text-[10px] font-bold text-slate-400 uppercase mb-0.5">Access Code</span><span className="font-mono text-sm font-bold text-slate-700">{patient.access_code}</span></div>
                      </div>
                      {viewMode === 'active' && <button onClick={() => setDischargeTarget(patient)} className="flex items-center gap-2 px-8 py-3 bg-white text-orange-600 border border-orange-100 rounded-[1.25rem] text-xs font-black uppercase tracking-widest shadow-sm hover:bg-orange-50 transition-all active:scale-95 font-sans"><Archive size={18} /> Discharge</button>}
                    </div>
                  </div>
                )}
                  ...
                  {STAGES.map((stage) => {
                    ...
                    return (
                      <button key={stage.id} onClick={() => { if(viewMode === 'active') api.updateStage(patient.id, stage.id as StageId, doctor.id) }} disabled={viewMode === 'discharged'} className={`flex flex-col items-center justify-center p-5 rounded-[1.5rem] border-2 transition-all ${isActive && viewMode === 'active' ? `${stage.color} border-transparent text-white shadow-xl scale-[1.03]` : 'bg-white border-slate-100 text-slate-400 hover:text-slate-900 hover:bg-slate-50 shadow-sm'} ${viewMode === 'discharged' ? 'opacity-40 cursor-not-allowed' : ''}`}>
                        ...
                        <span className="text-[11px] font-black uppercase tracking-tighter leading-tight font-sans">{stage.label}</span>
                      </button>
                    );
                  })}
                </div>
